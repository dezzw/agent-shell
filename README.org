...
Drag a file from your file manager (or =dired=) onto a shell or viewport buffer to attach it to the prompt, the same way a pasted image is. Images get a preview; any other file is attached as an =@path= link.
...
- Help menu (=?=) =f= - Set model config (ACP =model_config=, or Cursor's =fast= toggle).

The current selections are shown in the header right after the agent name; click any segment to open a popup menu. Model config segments (for example Composer Fast/Off) appear beside the model when the agent advertises them. The thought level segment only appears for models that expose it (e.g. Claude Sonnet/Opus with effort levels; Codex's reasoning_effort).

From the viewport (=agent-shell-viewport-view-mode= / =agent-shell-viewport-edit-mode=), the same controls are bound to:

- View mode: =v= (model), =F= (model config), =s= (mode), =t= (thought level).
- Edit mode: =C-c C-v=, =C-c C-f=, =C-c C-m=, =C-c C-t=.
...
|                                                       | agent-shell-set-session-model-config                    | Set a model-config option (for example Cursor fast) for the session.          |
...
| F / C-c C-f                                           | agent-shell-viewport-set-session-model-config           | Set a model-config option (for example Cursor fast) for the session.          |
